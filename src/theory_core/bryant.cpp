	...
	for (T_ITE_vec::iterator f = ITE_vec.begin(); f != ITE_vec.end(); f++){
		if (!(*f).isVar()) 
		    if (Creation_map.find((*f).getOpExpr()) == Creation_map.end()) { 
		       Creation_map.insert(pair< Expr, vector<Expr>* >( (*f).getOpExpr(), new vector<Expr>()));
		       Creation_map[(*f).getOpExpr()]->push_back((*f));
	           if (instance_map[(*f).getOpExpr()] < LIMIT || !(*f).isTerm())
			       ITE_map.insert(pair<Expr, Expr>((*f), name_map.find(pair<Expr, Expr>((*f).getOpExpr(), (*f)))->second));
			   else {
			   ...
			   Expr NewValue = name_map.find(pair<Expr, Expr>((*f).getOpExpr(), (*f)))->second;
		       Expr Add = ITE_generator((*f), NewValue, Creation_map, name_map, ITE_map);
...
void ExprTransform::GetPEqs(const Expr& e, B_name_map& name_map, set<Expr>& P_constrained_set, set<Expr>& Constrained_set, T_generator_map& Constrained_map, set<Expr>& SeenBefore) {
     if (!SeenBefore.insert(e).second)
		 return;
	 if (e.isEq())
         if (Constrained_set.find(e[1]) != Constrained_set.end() && Constrained_set.find(e[0]) != Constrained_set.end()) {
             Expr Temp0, Temp1;
			 if (e[0] != e[1]) {
			 if (e[0].isVar())
                 Temp0 = e[0];
			 else Temp0 = name_map.find(pair<Expr, Expr>((e[0]).getOpExpr(),(e[0])))->second;
			 if (e[1].isVar())
				 Temp1 = e[1];
			 else Temp1 = name_map.find(pair<Expr, Expr>((e[1]).getOpExpr(),(e[1])))->second;
			 Expr Eq = Temp0.eqExpr(Temp1);
			 Expr Reflexor = Temp1.eqExpr(Temp0);
			 Eq = Eq.notExpr();
			 Reflexor = Reflexor.notExpr();
			 ...
			 P_constrained_set.insert(Eq);
			  ...
			  if (Constrained_map.find(e[0]) == Constrained_map.end())
			  	 Constrained_map.insert(pair<Expr, set<Expr>*>(e[0], new set<Expr>));
	          Constrained_map[e[0]]->insert(e[1]);
                 ...
                 if (Constrained_map.find(e[1]) == Constrained_map.end())
				     Constrained_map.insert(pair<Expr, set<Expr>*>(e[1], new set<Expr>));
